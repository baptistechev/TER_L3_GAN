...
#             d_l_r = discriminator_model.train_on_batch(x, classes)
        ...
        if not i % 10: # 2x faster than testing each step...
            K.set_learning_phase(0) # 0 = test
            test_size = 20 # test using all of the actual fraud data
            x = get_data_batch(train, test_size, seed=i)
            z = np.random.normal(size=(test_size, rand_dim))
            if with_class:
                labels = x[:,-label_dim:]
...
# arguments : 
#   rand_dim = dimension bruit entrée
#   nb_step = nombre epoch ??
#   batch_size = taille des batch
#   k_d = ncritics, nombre d'itération du discriminateur pour une itération
#   k_g = nombre d'itération du generateur pour une itération
#   critic_pre_train_steps = nb iteration d'entrainement du discriminateur avant de commencer
#   log_interval = nb d'interval au bout duquel on produit un sample
#   learning_rate = taux apprentissage
#   base_n_count ?
#   data_dir = fichier des data produits?
#   generator/discrimintaor_model_path = lien vers des modèles déja entrainés ?
#   show = True/False verbose ?
